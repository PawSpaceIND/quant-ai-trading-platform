...
test("research lab and operator notes are excluded before cloud snapshot storage",async()=>{
 ...
 snapshots["/api/workspace"]={tenantId:"india-paper",researchLab:{private:"RESEARCH_SENTINEL"},researchPortfolio:{private:"PORTFOLIO_SENTINEL"},companyEvents:{private:"EVENT_SENTINEL"},audit:["NOTES_SENTINEL"],runtime:{status:"running"}};
 ...
 assert.equal(response.status,200);assert.equal(stored["/api/workspace"].researchLab,undefined);assert.equal(stored["/api/workspace"].researchPortfolio,undefined);assert.equal(stored["/api/workspace"].companyEvents,undefined);assert.deepEqual(stored["/api/workspace"].audit,[]);
