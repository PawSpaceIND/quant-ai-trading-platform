...
function BrokerObservationBody({state,onAsk}:{state?:BrokerObservationState;onAsk:(prompt:string)=>void}) {
  ...
  return <section className="panel portfolio-replay" aria-label="Broker order and trade observation">
    ...
    {report&&inspection&&<>
      ...
      <div className="replay-selectors"><label>Search broker orders<input aria-label="Search broker orders" value={query} placeholder="Symbol, order, exchange or product" onChange={e=>{setQuery(e.target.value);setPage(0);}}/></label><label>Status<select aria-label="Broker order status" value={filter} onChange={e=>{setFilter(e.target.value);setPage(0);}}><option value="all">All statuses</option>{[...new Set(report.orders.map(o=>o.status))].sort().map(s=><option key={s}>{s}</option>)}</select></label></div><div className="research-actions"><a href={report&&state?.journal?`/api/broker/observation?journalId=${state.journal.journalId}&sequence=${state.journal.selectedSequence}&captureSha256=${report.sha256}`:"/api/broker/observation"}>Export evidence</a><button onClick={()=>onAsk("Explain the broker order/trade observation status, its issue codes, and what remains unverified before pilot launch. Keep this external observation separate from our paper-account records.")}>Ask Atlas</button></div>
