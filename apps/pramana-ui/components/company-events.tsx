...
function MappingEditor({event, symbols, history, refresh}: {event: CompanyEvent; symbols: string[]; history: EventMapping[]; refresh: () => Promise<void>}) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const result = await request("/api/company-events", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({title: event.title, symbol, provenance: reference, reviewConfirmed: confirmed, expectedVerifiedAt: event.mapping?.verifiedAt ?? null})});
      setMessage(`Mapping recorded at ${when(result.mapping.verifiedAt)}. Earlier records remain in history.`); setReference(""); setConfirmed(false);
    } catch (error) {setMessage(`${error instanceof Error ? error.message : "Could not confirm saving."} Refresh and inspect history before retrying.`);}
  ...
  return <details className="event-mapping"><summary>Review company-to-symbol mapping</summary>
    ...
    <form onSubmit={save}>
      <label>NSE instrument<select aria-label="Event mapping instrument" required value={symbol} onChange={e => setSymbol(e.target.value)} disabled={busy}><option value="">Select an observed NSE instrument</option>{symbols.map(s => <option key={s}>{s}</option>)}</select></label>
      <label>Reviewed instrument reference<textarea aria-label="Reviewed instrument reference" required maxLength={1000} value={reference} onChange={e => setReference(e.target.value)} disabled={busy} placeholder="Record the source and exact company/instrument match you checked." /></label>
      <label className="event-confirm"><input type="checkbox" required checked={confirmed} onChange={e => setConfirmed(e.target.checked)} disabled={busy} /> I checked this exact company against that instrument reference.</label>
      <button disabled={busy || !symbols.length || !confirmed || !reference.trim() || !symbol}>{busy ? "Saving mapping…" : "Record mapping"}</button>
      {!symbols.length && <p className="muted">No NSE instruments are available from the configured market list.</p>}
      {message && <p role="status">{message}</p>}
    </form>
    <h4>Mapping history</h4>{history.length ? <ol>{[...history].reverse().map(m => <li key={m.verifiedAt}><strong>{m.symbol}</strong> · {when(m.verifiedAt)}<p>{m.provenance}</p></li>)}</ol> : <p>No mapping recorded.</p>}
  </details>;
}
export function CompanyEventsPanel({state, symbols, favorites, onAsk, onRefresh}: {state?: CompanyEventsState; symbols: string[]; favorites: string[]; onAsk: (question: string, asOf?: string) => void; onRefresh: () => Promise<void>}) {
  ...
  return <section className="panel company-events-panel">
    ...
    {!data || data.status !== "available" ? <p className="empty">{data?.detail || "Company-event storage has not been configured."}</p> : <>
      ...
      <div className="event-controls"><label>Known at<input aria-label="Company evidence cutoff" type="datetime-local" value={at} onChange={e => setAt(e.target.value)} /></label><button disabled={busy || !at} onClick={() => void loadTime()}>{busy ? "Loading…" : "Apply time"}</button><button onClick={() => {setHistorical(null); setAt(""); setError(""); void onRefresh();}}>Current evidence</button><a href={`/api/company-events?download=1${historical ? `&at=${encodeURIComponent(historical.asOf)}` : ""}`} download>Export event evidence ↓</a></div>
      {error && <p role="alert">{error}</p>}
      <div className="event-workspace"><div><div className="event-list" aria-label="Company event results">{filtered.slice(page * 10, page * 10 + 10).map(e => <button key={e.id} aria-pressed={e.id === event?.id} onClick={() => setSelected(e.id)}><strong>{e.title}</strong><span>{e.mapping?.symbol || "Unmapped"} · {e.ambiguous ? "Conflicting revisions" : e.captureKind === "imported" ? "Imported" : "Recorded HTTPS capture"}</span><span>First seen {when(e.firstSeenAt)}</span></button>)}</div>{!filtered.length && <p>No announcements match these filters.</p>}
      ...
      </div>{event && <article className="event-detail"><h3>{event.title}</h3><p>{event.mapping?.symbol || "No reviewed symbol mapping"}</p><p>{event.description || "No description supplied."}</p>
        ...
        {(event.ambiguous || !event.mapping) && <p className="research-notice">{event.ambiguous ? "Conflicting revisions share the same first-seen time." : "This company has no mapping at this cutoff."} Excluded from automatic symbol-specific Atlas context.</p>}
        ...
        {historical ? <p className="muted">Return to current evidence to record a mapping. Historical views never backdate a review.</p> : <MappingEditor key={`${event.id}:${event.mapping?.verifiedAt || "unmapped"}`} event={event} symbols={symbols} history={data.mappings.filter(m => m.title === event.title)} refresh={onRefresh} />}
