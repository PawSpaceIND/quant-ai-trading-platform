...
export function MarketMonitor() {
  ...
  return <section className="rounded border border-slate-800 bg-slate-950/50 p-5 text-slate-300">
    ...
    {data.coverage?.groups?.length ? <section className="mt-5 rounded border border-slate-800 bg-slate-900/60 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] uppercase tracking-[0.2em] text-cyan-300">INDIA COVERAGE</p><h3 className="mt-1 text-base font-semibold text-slate-200">{data.coverage.scope}</h3></div><span className="rounded border border-emerald-800 px-2 py-1 text-xs text-emerald-300">Paper only</span></div><div className="mt-3 grid gap-2 md:grid-cols-2">{data.coverage.groups.map(group => <article key={group.id} className="rounded border border-slate-800 p-3"><div className="flex items-start justify-between gap-2"><strong className="text-sm text-slate-200">{group.label}</strong><span className={`rounded px-2 py-0.5 text-[10px] ${group.status === "observed" ? "bg-emerald-950 text-emerald-300" : "bg-amber-950 text-amber-300"}`}>{group.status === "observed" ? "Observed" : group.mode === "requires_contract" ? "Contract needed" : "Planned"}</span></div><p className="mt-1 text-xs text-slate-500">{group.exchange} · {group.currency} · {group.assetClasses.join(" / ")}</p><p className="mt-2 text-xs text-slate-300">{group.examples.join(" · ")}</p><p className="mt-2 text-xs leading-5 text-slate-500">{group.detail}</p></article>)}</div><p className="mt-3 text-xs leading-5 text-slate-500">{data.coverage.disclaimer}</p></section> : null}
