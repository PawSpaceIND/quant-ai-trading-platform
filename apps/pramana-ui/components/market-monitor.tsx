...
type Row = { symbol: string; available: boolean; price?: number; change?: number | null; volume?: number; lastTrade?: string; exchangeTimestamp?: string; history?: {date: string; close: number}[]; instrument?: {market: string; assetClass: string; currency: string; exchange: string; providerExchangeToken?: string; contract?: string; expiry?: string; underlying?: string; optionType?: string; strike?: number; product?: string; segment?: string; lotSize?: number; tickSize?: number} };
type Snapshot = { instrumentUniverse?: import("@/lib/market").InstrumentUniverse; news?: {headline: string; publishedAt: string; source: string}[]; runtime?: {status: string; updatedAt?: string; halted?: boolean; cadenceFailures?: number; watchlist?: string[]}; status: string; fetchedAt?: string; session?: string; rows: Row[]; commodity?: string; note?: string; providers?: Record<string,string>; coverage?: IndiaCoverage };
const instrumentLabel = (instrument: Row["instrument"]) => {
  if (!instrument) return "Venue not supplied";
  const parts = [
    instrument.exchange,
    instrument.currency,
...
export function MarketMonitor() {
  ...
  return <section className="rounded border border-slate-800 bg-slate-950/50 p-5 text-slate-300">
    ...
    <div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b border-slate-700"><th className="p-2">Instrument</th><th>Last price</th><th>Change vs previous close</th><th>Volume</th><th>Recent daily closes</th><th>Last trade / exchange time</th></tr></thead><tbody>{data.rows.map(row => {
      ...
      return <tr key={(row.instrument?.exchange || "UNKNOWN") + ":" + row.symbol} className="border-b border-slate-800"><td className="p-2 font-medium">{row.symbol}<span className="block text-xs text-slate-500">{instrumentLabel(row.instrument)}</span></td><td>{row.available ? row.price?.toLocaleString("en-IN") : "Unavailable"}</td><td>{row.change == null ? "—" : row.change.toFixed(2)+"%"}</td><td>{row.volume?.toLocaleString("en-IN") ?? "—"}</td><td>{values.length>1 ? <svg viewBox="0 0 140 32" width="140" height="32" role="img" aria-label={row.symbol+" historical daily closing prices"}><polyline fill="none" stroke="#22d3ee" strokeWidth="1.5" points={values.map((v,i)=>(i*140/(values.length-1))+","+(30-(v-min)/span*28)).join(" ")} /></svg> : "No history"}</td><td className="text-xs">{row.lastTrade || row.exchangeTimestamp || "Not supplied"}</td></tr>;
    })}</tbody></table></div>
