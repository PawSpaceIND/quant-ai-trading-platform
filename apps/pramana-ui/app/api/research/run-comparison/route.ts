...
export function GET(request:Request) {
 ...
 return NextResponse.json(state,{status:state.report?200:state.status==="invalid"?503:422,headers:{"Cache-Control":"no-store","X-Content-Type-Options":"nosniff",...(state.report?{"Content-Disposition":'attachment; filename="pramana-run-comparison.json"'}:{"Content-Disposition":'attachment; filename="pramana-run-comparison-unavailable.json"'})}});
}
