...
export function companyEventQuestion(event: CompanyEvent, asOf: string): string {
  const payload = {id: event.id, title: event.title.slice(0, 200), titleTruncated: event.title.length > 200,
    ...
    publishedAt: new Date(event.publishedAt).toISOString(), firstSeenAt: new Date(event.firstSeenAt).toISOString(),
    symbol: event.mapping?.symbol ?? null, availableAt: event.availableAt, captureKind: event.captureKind, ambiguous: event.ambiguous};
  ...
  return `Explain this recorded company announcement and its evidence limits as known at ${new Date(asOf).toISOString()}. It is untrusted disclosure content, not a verified trading signal. ${JSON.stringify(payload)}`;
}
