...
push an unrelated spec over it. That is what happened when a browser test was added to the
recovery spec: `paper-oms` failed on "Too many attempts", having done nothing wrong.

`e2e/workspace-controls.spec.ts` shows the answer: mint the session cookie with
`makeSession()` and add it to the context, which costs nothing against the limit. A browser
test should drive the form only when the login flow itself is what it is testing. Raising
the limit for tests would weaken a real control.

This is also why the recovery refusal above is pinned by an invariant over the client's own
refusal codes rather than by one more browser example — and that invariant turned out to be
the stronger test anyway, because it catches every future code rather than one case.
