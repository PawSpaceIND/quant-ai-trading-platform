...
The existing operational recovery bundle has only one optional research-file slot. It does **not** automatically capture separate experiment, portfolio or company-event databases and all their published reports. Include these in a separately inventoried, consistent research archive with independently retained hashes until the recovery manifest supports them explicitly. Verify restoration and replay on the target host before closing recovery. A local export is not an off-host backup or recovery drill.
