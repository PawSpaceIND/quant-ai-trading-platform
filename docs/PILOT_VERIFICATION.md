...
## Current-state freshness follow-up

The workspace now re-evaluates current evidence at read time. Engine heartbeat, accepted per-instrument ticks, portfolio valuation, held marks, strategy manifest, protection, reconciliation and collector snapshots have explicit age and future bounds. Missing, future, expired, duplicate or malformed observations fail closed; a collector refresh cannot qualify an engine tick. Markets exposes the heartbeat, source timestamp, age and rejection reason for every validated instrument. [Freshness contract](FRESHNESS.md).

Local verification passes **716 Python, 101 dashboard and 13 Worker tests**, required/changed-script Ruff, TypeScript and the private Webpack production build. The authenticated production smoke verified the stale heartbeat/tick explanation, current API/Atlas context and the 390px engine-feed panel without document overflow or browser warnings. Historical run-comparison data remains available after current workspace expiry. These are synthetic checks; open-session feed, target-host operations and strategy acceptance remain required.

The external-gate preflight is also fail-closed: a document is accepted only when X01/X02/X03 each contain all named evidence items, a reviewer, timezone-aware observation time, target host, release revision and non-empty attachments. Missing evidence returns a non-ready report and cannot enable live execution. The preflight itself does not claim that those external observations exist.
