...
## Recorded paper/replay phases

The `run-comparison` and `run-comparison-restored` phases use actual harness and paper telemetry records from synthetic inputs, independent of the active protection fixture. Both production dashboard instances check 70 expected minutes, three explicit gaps, withheld metrics, two fills per run, private selected export, hash replacement/corruption rejection and isolated saved Atlas context. A separate `runComparisonRecovery` checkpoint snapshots the replay ledger, compares its logical/run evidence and regenerates an equivalent comparison from the restored database. Runtime provider access remains disabled. Schema 2 bundle recovery is additionally exercised by Python tests.

The `run-comparison` and `run-comparison-restored` phases now use 43,200 older synthetic minute records plus 69 selected observations. Their artifact records selection counts, preserved comparison gaps, selected export/Atlas context and current-performance withholding/recovery after a deliberately invalid stored timestamp. The current-history mutation is restored in a `finally` block and does not change the previously captured research report. See [retained history](OBSERVATION_HISTORY.md).
