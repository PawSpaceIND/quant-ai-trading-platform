...
* `evidence_files` maps each pinned digest to the retained files behind it, as paths relative to the review document. The gate recomputes every digest with `quant_ai.operations.pilot_gate.evidence_bundle_digest` - the same hasher the external-gate preflight uses - and refuses when the recomputed digest does not match. A digest over files that are not there is refused, not accepted.
