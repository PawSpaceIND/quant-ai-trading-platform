"""Reuse paired control/mutation/restoration verification for read-only budget guards."""
...
INVENTORY = ROOT / "docs/evidence/nse-watchlist-budget-guard-inventory.json"
CASES = json.loads(INVENTORY.read_text())["mutations"]
PROTECTED = ("scripts/inspect_watchlist_budget.py", "tests/test_nse_watchlist_budget.py")
LAUNCHER = """
import sys

def deny_network(event, args):
    if event in ('socket.connect', 'socket.connect_ex', 'socket.getaddrinfo', 'socket.sendto'):
        raise RuntimeError('offline_budget_verification_network_forbidden')
...
def test_budget_guard_control_mutation_and_restoration(tmp_path, case):
    before = hashes()
    source = (ROOT / case["path"]).read_text()
    assert source.count(case["old"]) == 1, f"Changed mutation anchor: {case['id']}"
    work = tmp_path / "copy"
    work.mkdir()
    ...
    (work / "tests").mkdir()
    shutil.copyfile(ROOT / PROTECTED[1], work / PROTECTED[1])
