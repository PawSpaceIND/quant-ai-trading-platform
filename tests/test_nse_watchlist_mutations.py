...
INVENTORY = ROOT / "docs/evidence/nse-watchlist-guard-inventory.json"
...
def deny_network(event, args):
    if event in ('socket.connect', 'socket.connect_ex', 'socket.getaddrinfo', 'socket.sendto'):
        raise RuntimeError('offline_watchlist_verification_network_forbidden')
...
def test_nse_watchlist_guard_requires_passing_control_and_failing_mutant(tmp_path, case):
    before = hashes()
    source = (ROOT / case["path"]).read_text()
    assert source.count(case["old"]) == 1, f"Review changed anchor: {case['id']}"
    work = tmp_path / "copy"
    work.mkdir()
    for directory in ("src", "tests", "deploy", ".github", "scripts"):
