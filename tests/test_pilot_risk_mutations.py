...
INVENTORY = ROOT / "docs/evidence/pilot-risk-guard-inventory.json"
...
def deny_network(event, args):
    if event in ('socket.connect', 'socket.connect_ex', 'socket.getaddrinfo', 'socket.sendto'):
        raise RuntimeError('offline_risk_verification_network_forbidden')
...
def test_pilot_risk_guard_requires_passing_control_and_failing_mutant(tmp_path, case):
    before = hashes()
